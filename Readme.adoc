...
* Gets imgui_bundle, from the first available of:
** an installed imgui_bundle package (`cmake --install` of imgui_bundle, or a package manager such as Conan): pass `-DCMAKE_PREFIX_PATH=/path/to/imgui_bundle_install` to cmake
** a copy of imgui_bundle in `external/imgui_bundle`
** a download of imgui_bundle at configure time (default)
...
===== Alternative: build against an installed imgui_bundle

If imgui_bundle was installed (`cmake --install` from an imgui_bundle build, or via a package manager),
point CMake to the install prefix instead of cloning or downloading imgui_bundle:

[source, bash]
----
mkdir -p build && cd build
cmake .. -DCMAKE_PREFIX_PATH=/path/to/imgui_bundle_install
make -j 4
----
